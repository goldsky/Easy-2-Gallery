...
function restore($path, $pid) {
    ...
    if (FALSE !== $fs) {
        ...
        foreach ($fs as $f) {
            ...
            if ($name == '_thumbnails' && $name != $basee2gdir && $name != '')
                continue;
            ...
            if (is_validfolder($f)) {
                if (isset($odirs[$name])) {
                    ...
                    if (!rename($f, $nf))
                        $_SESSION['easy2err'][] = "Could not rename path " . $f;
                    else {
                        $_restore['d']++;
                        @chmod($nf, 0755);
            ...
            if (is_validfile($f)) {
                ...
                elseif ($fbasename == $ofiles[$trimmedname]['id'] . $ext
                    ...
                    if (!rename($f, $nfilename))
                        $_SESSION['easy2err'][] = $lngi['rename_file_err'];
                    else {
                        $_restore['f']++;
...
/**
 * delete all dirs/files under the specified path
    ...
    foreach ($fs as $f) {
        ...
        if (is_validfolder($f)) {
            if (isset($odirs[$name])) {
                ...
                if (!restore_all($nf . '/', $odirs[$name]['id']))
                    return FALSE;
...
if (isset($_GET['p']) && $_GET['p'] == 'del_inst_dir') {
    ...
    if (!isset($tab[$GLOBALS['table_prefix'] . 'easy2_dirs'])) {
        if (mysql_query('CREATE TABLE IF NOT EXISTS ' . $GLOBALS['table_prefix'] . 'easy2_dirs (
                        ...
                        cat_summary varchar(255) NULL default \'\',
    ...
    // cat_visible
    if (check_field($GLOBALS['table_prefix'] . 'easy2_dirs', 'cat_visible') !== FALSE) {
    ...
    // additional field for 1.4.0 RC1
    // cat_summary
    if (check_field($GLOBALS['table_prefix'] . 'easy2_dirs', 'cat_summary') === FALSE) {
        mysql_query('ALTER TABLE ' . $GLOBALS['table_prefix'] . 'easy2_dirs ADD cat_summary varchar(255) default NULL AFTER cat_alias');
        $_SESSION['easy2suc'][] = $lngi['field'] . ' ' . $GLOBALS['table_prefix'] . 'easy2_dirs.cat_summary ' . $lngi['created'];
    }
    ...
    // additional field for 1.4.0 RC1
    // summary
    if (check_field($GLOBALS['table_prefix'] . 'easy2_files', 'summary') === FALSE) {
        mysql_query('ALTER TABLE ' . $GLOBALS['table_prefix'] . 'easy2_files ADD summary varchar(255) default NULL AFTER name');
        $_SESSION['easy2suc'][] = $lngi['field'] . ' ' . $GLOBALS['table_prefix'] . 'easy2_files.summary ' . $lngi['created'];
    }
    ...
    if (empty($plugin_id) || $plugin_id == '') {
        ...
        if (mysql_num_rows($query) == 0) {
            ...
            $plugin_id = mysql_result($query, 0, 0);
    }
    else
        $plugin_id = $e2g['plugin_id'];
    ...
    if (!empty($plugin_id)) {
        ...
        if ($delete) {
            ...
            $_SESSION['easy2err'][] = __LINE__ . ' Error: ' . mysql_error();
    }
    ...
    if (empty($e2g['snippet_id']) || $e2g['snippet_id'] == '') {
        ...
        if (mysql_num_rows($select) > 0)
            $snippet_id = mysql_result($select, 0, 0);
    ...
    if (empty($e2g['plugin_id']) || $e2g['plugin_id'] == '') {
        ...
        if (mysql_num_rows($select) > 0)
            $plugin_id = mysql_result($select, 0, 0);
        mysql_free_result($select);
    }
    else
        $plugin_id = $e2g['plugin_id'];
