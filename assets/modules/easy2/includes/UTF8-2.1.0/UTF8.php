...
class UTF8
{
	...
	*/
	...
	#autogenerated by unicode_blocks_txt2php() PHP function at 2009-11-10 18:40:37, 171 blocks total
	 ...
	 * Восстанавливает диакритические знаки в тексте, при условии, что их байтовые позиции и кол-во символов не изменились!
	 *
	 ...
	 * <br />The function to convert data of any structure from any encoding to UTF-8
     *
	 ...
	 * <br />The function to convert an arbitrary data structure for encoding UTF-8 encoding.
     *
	 * @param   array|scalar|null  $data
	 * @param   string             $charset
	 * @return  array|scalar|null  returns FALSE if error occured
	...
	public static function ltrim($s, $charlist = null)
	{
		...
		return preg_replace('~^[' . preg_quote($charlist, '~') . ']+~uSX', '', $s);
	}
	...
	public static function trim($s, $charlist = null)
	{
		...
		$charlist_re = preg_quote($charlist, '~');
	...
	public static function str_pad($s, $length, $pad_str = ' ', $type = STR_PAD_RIGHT)
	{
		...
		{
			$repeat_num = ceil($pad_len / $pad_str_len);
			return self::substr(str_repeat($pad_str, $repeat_num), 0, floor($pad_len)) . $s;
		}
		...
		if ($type == STR_PAD_BOTH)
		{
			...
			$padding_right = self::substr(str_repeat($pad_str, $repeat_times_right), 0, $pad_amount_left);
	 ...
	 * Т. о. метод работает достаточно надёжно.<br />
     * 
     * Recode text files in the specified folder in the UTF-8<br />
     * When processing skipped binary files, files are encoded in UTF-8 and the files that encode failed.
     * T. o. method works quite reliably.
     *
	 * @param   string       $dir           директория для сканирования
	 * @param   string|null  $files_re      регул. выражение для шаблона имён файлов,
	 *                                      например: '~\.(?:txt|sql|php|pl|py|sh|tpl|xml|html|xhtml|phtml|htm|js|json|css|conf|cfg|ini)$~sSX'
	 ...
	 */
	public static function convert_files_from($dir, $files_re = null, $is_recursive = true, $charset = 'cp1251')
	{
		if (! ReflectionTypehint::isValid()) return false;
		$dh = opendir($dir);
		...
		while (($name = readdir($dh)) !== false)
		{
			...
			{
				$c = self::convert_files_from($file, $files_re, $is_recursive, $charset);
